...
file_handler = logging.FileHandler('rntn_train_validate_and_test_freebase_experiment.log')
        ...
        for i in range(1, num_epochs + 1):
            ...
            for j in range(0, len(indexed_training_data), batch_size):
                ...
                if j % (batch_size * 10) == 0:
                    ...
                    logger.debug(f'training cost: {cost_training}')
                    logger.debug(f'training accuracy: {accuracy_training}')

                iteration += 1

            cost_training, accuracy_training = evaluate_training(loss,
                                                                 feed_dict,
                                                                 batch_placeholders,
                                                                 label_placeholders,
                                                                 corrupt_placeholder,
            ...
            if accuracy_validation <= prev_accuracy_validation and i > 10 and stop_early:
                logger.info("Validation accuracy stopped improving, stopping training early after %d epochs!" % i)
                break
...
def evaluate_training(loss,
    ...
    cost_training, = sess.run([loss], feed_dict=feed_dict)
    accuracy_training = do_eval(sess,
                                eval_correct,
                                batch_placeholders,
                                label_placeholders,
    ...
    return cost_training, accuracy_training

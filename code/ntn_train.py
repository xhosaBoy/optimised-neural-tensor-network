...
def get_batch(batch_size, data, num_entities, corrupt_size, idx):
    ...
    batch = [(data[i][0], data[i][1], data[i][2], random.randint(0, num_entities - 1))
             for i in indices for j in range(corrupt_size)]
...
def fill_feed_dict_train(batches, train_both, batch_placeholders, label_placeholders, corrupt_placeholder):
    ...
    for i in range(len(batch_placeholders)):
        if batches[i]:
            ...
            feed_dict[label_placeholders[i]] = [[0.0]
                                                for j in range(len(batches[0]))] if len(batches[0]) else [[0.0]]
...
def run_training(
    ...
    logger.info("Load entities and relations...")
    # python list of (e1, R, e2) for entire training set in index form
    indexed_validation_data = data_to_indexed_validation(
        validation_data, entities_list, relations_list)
    ...
    with tf.Graph().as_default():
        ...
        batch_placeholders = [tf.placeholder(tf.int32, shape=(
            None, 3), name='batch_' + str(i)) for i in range(num_relations)]
        label_placeholders = [tf.placeholder(tf.float32, shape=(
            None, 1), name='label_' + str(i)) for i in range(num_relations)]
        ...
        for i in range(1, num_epochs + 1):
            ...
            for j in range(0, len(indexed_training_data), batch_size):
                ...
                if j % (batch_size * 10) == 0:
                    ...
                    logger.info(f'training cost: {cost_training}')
                    logger.info(f'training accuracy: {accuracy_training}')
        ...
        accuracy_validation = test(indexed_test_data,
                                   num_relations,
                                   inference_train,
                                   batch_placeholders,
                                   label_placeholders,

...
def run_evaluation():
    ...
    with tf.Graph().as_default():
        ...
        print('validation accuracy:', do_eval(sess,
                                              eval_correct,
                                              batch_placeholders,
                                              label_placeholders,
                                              corrupt_placeholder,
...
def getThresholds():
    ...
    predictions_list = ntn.inference(batch_placeholder,
                                     corrupt_placeholder,
                                     init_word_embeds,
                                     entity_to_wordvec,
                                     num_entities,
    ...
    while(score <= max_score):
        ...
        for i in range(params.num_relations):
            ...
            accuracy = tf.reduce_mean((predictions == dev_data[current_relation_list, 0]))
...
def getPredictions():
    ...
    for i in range(test_data.shape[0]):
        ...
        if(preictions_list[i, 0] <= best_thresholds[rel, 0]):
            predictions[i, 0] = 1
        else:
            predictions[i, 0] = -1

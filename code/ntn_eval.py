...
def run_evaluation():
  ...
  with tf.Graph().as_default():
    sess = tf.Session()
    batch_placeholders = [tf.placeholder(
        tf.float32, shape=(None, 3)) for i in range(num_relations)]
    label_placeholders = [tf.placeholder(
        tf.float32, shape=(None, 1)) for i in range(num_relations)]
    ...
    # saver.restore(sess, params.output_path + 'Wordnet100.sess')
    saver.restore(sess, checkpoint)
    #init = tf.initialize_all_variables()
    # sess.run(init)
    print('validation accuracy:', do_eval(sess, eval_correct, batch_placeholders, label_placeholders, corrupt_placeholder, batches, labels, batch_size))
...
def do_eval(sess, eval_correct, batch_placeholders, label_placeholders, corrupt_placeholder, test_batches, test_labels, num_examples):
  ...
  feed_dict = fill_feed_dict(test_batches, test_labels, params.train_both,
...
def getThresholds():
  ...
  predictions_list = ntn.inference(batch_placeholder, corrupt_placeholder, init_word_embeds,
  ...
  while(score <= max_score):
    ...
    for i in range(params.num_relations):
      ...
      predictions = (predictions_list[
                     current_relation_list, 0] <= score) * 2 - 1
